...
namespace ufo
{
  ...
  void raman_bec_extract(std::string config_file)
  {
    struct Config
    {
      std::string VaspOutputFile;
      std::vector<std::size_t> SelectedAtoms;
      std::optional<double> Charge;
    };
    ...
    auto file = biu::Hdf5file(config.VaspOutputFile);
    auto bec = file.read<std::vector<Eigen::Matrix3d>>("/results/linear_response/born_charges");
    auto epsilon = file.read<Eigen::Matrix3d>
      ("/results/linear_response/electron_dielectric_tensor");
    // 理想情况下，BEC 求和应该是一个对角矩阵，且对角元素全相等，等于原子电荷；但由于数值误差，可能会有一些偏移
    // 我们将这个偏移平均分配到每个原子上，使得它们求和后得到正确的值；如果没有提供电荷，则默认总和为0
    // VASP 应该已经做了这个修正，但存在 bug，我们需要再次修正
    auto ideal_charge = (config.Charge.value_or(0) * Eigen::Matrix3d::Identity()).eval();
    auto actual_charge = *std::ranges::fold_left_first
      (bec, [](auto&& a, auto&& b) { return (a + b).eval(); });
    auto offset = (actual_charge - ideal_charge) / bec.size();
    for (std::size_t k = 0; k < bec.size(); k++) bec[k] -= offset;

    auto print_matrix = [](const Eigen::Matrix3d& matrix)
    {
      std::stringstream ss;
      for (std::size_t i = 0; i < 3; i++)
        for (std::size_t j = 0; j < 3; j++)
          ss << "{:0.16f}\t"_f(matrix(i, j));
      return ss.str().substr(0, ss.str().size() - 1); // remove last tab
    };

    std::cout << "Generated by ufo raman_bec_extract\n";
    std::cout << "{}\n"_f(print_matrix(epsilon));
    for (std::size_t i = 0; i < bec.size(); i++)
    {
      if (std::ranges::find(config.SelectedAtoms, i) == config.SelectedAtoms.end()) continue;
      std::cout << "{}\n"_f(print_matrix(bec[i].transpose()));
    }
  }
}
