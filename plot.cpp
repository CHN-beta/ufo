...
int main()
{
  ...
  // 构建未涂抹时的数据
  std::vector<std::vector<double>> original_data(31, std::vector<double>(3501));
  ...
  for (auto qpoint: output.QPointData)
    for (auto mode: qpoint.ModeData)
      if (qpoint.QPoint[1] < 1e-3 && qpoint.QPoint[2] < 1e-3)
        original_data.at(static_cast<int>(qpoint.QPoint[0] * 30 + 0.5)).at(static_cast<int>(mode.Frequency / 0.01 + 500 + 0.5)) += mode.Weight;
  ...
  // 输出
  for (int i = 0; i < 31; i++)
    os << fmt::format("\t{}", static_cast<double>(i) / 30);
  os << "\n";
